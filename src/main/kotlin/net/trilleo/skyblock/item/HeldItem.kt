 ...
 * once per frame per hand, and [SkyblockItem.attributes] deep-copies the item's tag — asking it directly
 ...
 * change the custom-data `id` or `uuid`. So a reference compare is the whole check, and it costs nothing.
 *
...
object HeldItem {
    ...
    private fun read(stack: ItemStack) {
        ...
        val extra = SkyblockItem.attributes(stack)
        if (extra == null) {
            id = null
            uuid = null
            return
