...
**There is no setting to change.** The mod follows Minecraft's own language, so picking a language in
**Options → Language** switches Hex's menus, tooltips, keybind names and reminder presets along with the rest of the
game. Anything a translation has not caught up with falls back to English rather than going blank.
